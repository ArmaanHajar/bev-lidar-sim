...
Keys in the live window:
    space  pause / resume
    1/2/3  safe / normal / daring automated driving profile
...
The ego car (cyan) follows changing routes through a generated city district,
obeying signals and all-way stops while other traffic moves between its own
...
def main() -> None:
    ...
    ap.add_argument("--scenario",
                    choices=("city", "suburbs", "riverside", "roadworks",
                             "arterial"),
                    default="city",
                    help="driving scenario to run (default: city)")
    ap.add_argument("--driver", choices=("safe", "normal", "daring"),
                    default="normal", help="ego driving style (default: normal)")
    args = ap.parse_args()
    ...
    if args.scenario == "arterial":
        from ..sim.arterial import ArterialSimulator
        sim = ArterialSimulator(seed=args.seed, driving_mode=args.driver)
    else:
        from ..sim.scenarios import make_sim
        sim = make_sim(args.scenario, seed=args.seed,
                       driving_mode=args.driver)
    ...
    paused, boxes, rays = False, False, True
    running = True
    while running:
        for ev in pygame.event.get():
            if ev.type == pygame.QUIT:
                ...
                if ev.key in (pygame.K_ESCAPE, pygame.K_q):
                    ...
                    sim.set_driving_mode("safe")
                    sim.manual = False
                elif ev.key == pygame.K_2:
                    sim.set_driving_mode("normal")
                    sim.manual = False
