...
from ...maps.roadgraph import (ConnectorDef, LaneDef, RoadDef, RoadGraph,
                               RoadNode)
...
def add_two_way_road(graph: RoadGraph, a_id: str, b_id: str,
    ...
    graph.roads.append(RoadDef(f"road:{a_id}={b_id}", center, ROAD_HALF))
...
def build_grid_roadgraph(xs=GRID_COORDS, ys=GRID_COORDS,
                         ...
                         with_signals: bool = True) -> RoadGraph:
    ...
    turn, so T-junctions have to be stop-controlled. Since signals permit
    only straight movements, heavily-skipped maps should also pass
    `with_signals=False` (all-way stops everywhere) or missions through the
    gaps may become unroutable.
    """
    ...
    for i, x in enumerate(xs):
        for j, y in enumerate(ys):
            ...
            kind = ("signal" if with_signals and (i + j) % 2 == 0
                    and degree == 4 else "stop")
