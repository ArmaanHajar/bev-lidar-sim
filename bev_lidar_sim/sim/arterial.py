"""Legacy arterial scenario: a small signalized east-west strip.

The world is an east-west arterial crossed by three real side streets: two
signalized intersections and one all-way stop. Vehicles enter and leave the
arterial by driving real right-turn arcs to/from the side streets (with turn
signals), instead of teleporting; the ego wraps around the eastbound lane
forever. Run it with `drive.py --scenario arterial`.

This scenario predates the generated city and keeps a mechanism the city no
longer needs: `turn_at_u` diverts a vehicle onto its route mid-path instead
of at the path end. The shared `Simulator.step` supports both forms.
...
from .traffic import (REFL, AllWayStop, Path, Signal, Simulator)
...
# --- Scenario --------------------------------------------------------------
class ArterialSimulator(Simulator):
