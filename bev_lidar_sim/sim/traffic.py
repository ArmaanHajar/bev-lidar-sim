...
# IDM parameters for background traffic, plus selectable ego-driver profiles.
IDM = dict(a=1.5, b=2.2, s0=2.0, T=1.4, delta=4.0)

DRIVER_PROFILES = {
    "safe": dict(a=1.25, b=2.0, s0=2.8, T=1.8, delta=4.0,
                 speed_scale=0.90, turn_speed=4.2),
    "normal": dict(a=1.8, b=2.4, s0=2.0, T=1.3, delta=4.0,
                   speed_scale=1.12, turn_speed=5.2),
    "daring": dict(a=2.4, b=3.0, s0=1.4, T=0.95, delta=4.0,
                   speed_scale=1.30, turn_speed=6.3),
}


def idm_accel(v: float, v0: float, gap: float, dv: float,
              params=IDM) -> float:
    """Longitudinal acceleration from the Intelligent Driver Model."""
    a, b, s0, T = params["a"], params["b"], params["s0"], params["T"]
    if not math.isfinite(gap):
        return a * (1.0 - (v / max(v0, 0.1)) ** params["delta"])
    gap = max(gap, 0.1)
...
class Simulator:
    ...
    def __init__(self, seed: int = 1, driving_mode: str = "normal"):
        ...
        self.signals: dict = {}
        self.stop_mgrs: dict = {}
        self.set_driving_mode(driving_mode)

    def set_driving_mode(self, mode: str) -> None:
        """Select the ego's automated driving style."""
        if mode not in DRIVER_PROFILES:
            choices = ", ".join(DRIVER_PROFILES)
            raise ValueError(f"unknown driving mode {mode!r}; choose {choices}")
    ...
    def step(self, dt: float) -> None:
        ...
        for veh in self.vehicles:
            ...
            speed_scale = 1.0
            if veh.is_ego and not self.manual:
                params = DRIVER_PROFILES[self.driving_mode]
                turn_speed = params["turn_speed"]
                speed_scale = params["speed_scale"]
            v0 = min(veh.v0, veh.path.speed) * speed_scale
            acc = idm_accel(veh.v, v0, gap, dv, params)
