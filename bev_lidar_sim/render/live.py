...
def _draw_city_world(surf, sim, fps=None):
    ...
    for pts in sim.world.water_polys:
        if _poly_visible(pts, bounds):
            pygame.draw.polygon(surf, WATER, [cam.pt(*p) for p in pts])

    # Sidewalk strips sit below asphalt so crossings and corners read clearly.
    for pts in sim.world.sidewalk_polys:
        if _poly_visible(pts, bounds):
            pygame.draw.polygon(surf, SIDEWALK, [cam.pt(*p) for p in pts])
    for pts in sim.world.road_polys:
        if _poly_visible(pts, bounds):
            pygame.draw.polygon(surf, ASPHALT, [cam.pt(*p) for p in pts])

    for a, b in sim.world.lane_dashes:
        if _segment_visible(a, b, bounds):
            pygame.draw.line(surf, LANE_YELLOW, cam.pt(*a), cam.pt(*b), 2)
    for quad in sim.world.crosswalk_quads:
        if _poly_visible(quad, bounds):
            pygame.draw.polygon(surf, MARK_WHITE,
                                [cam.pt(*p) for p in quad])
    ...
    for box in sim.world.barriers:
        if _box_visible(box, bounds, margin=3.0):
            _draw_barrier(surf, cam, box)
...
def _poly_visible(pts, bounds):
    xs, ys = pts[:, 0], pts[:, 1]
    return xs.max() >= bounds[0] and xs.min() <= bounds[2] \
        and ys.max() >= bounds[1] and ys.min() <= bounds[3]


def _draw_barrier(surf, cam, box):
    """Construction barrier: hazard-orange board with white stripes."""
    pts = [cam.pt(*p) for p in box.corners()]
    _poly(surf, pts, _hex("#d9822b"), _hex("#8a4f16"))
    c, s = math.cos(box.heading), math.sin(box.heading)
    n = max(int(box.length / 1.4), 1)
    for k in range(n):
        f = (k + 0.5) / n - 0.5
        sx = box.cx + f * box.length * c
        sy = box.cy + f * box.length * s
        px, py = cam.pt(sx, sy)
        pygame.draw.line(surf, MARK_WHITE,
                         (px - 2, py + 3), (px + 2, py - 3), 2)
...
def _draw_hud(surf, sim, fps=None):
    ...
    if fps is not None:
        _text(surf, "%3.0f fps" % fps, (270, 42), _hex("#5c6b80"), size=11,
              mono=True, anchor="topright")
    _text(surf, sim.driving_mode.upper() + " MODE", (270, 61),
          _hex("#5c6b80"), size=10, mono=True, anchor="topright")
    if getattr(sim, "ego_od", None):
        goal = "%d m -> %s" % (sim.ego_remaining(), sim.ego_od[1])
        _text(surf, goal, (270, 76), _hex("#7ea4c9"), size=10, mono=True,
              anchor="topright")
    keys = "space pause   1 safe  2 normal  3 daring   b boxes   r rays   m manual"
